...
describe("oauthLogin", () => {
  test("prints the URL, awaits the attempt, disposes it, returns the token", async () => {
    ...
    expect(token).toBe("user@x:fake-token");
  ...
  // Regression: disposing the attempt stub is upstream's
  ...
  test("does not send the abandon signal while waiting (real rpc pair)", async () => {
    ...
    try {
      ...
      attempt.complete("user@x:fake-token");
      await expect(pending).resolves.toBe("user@x:fake-token");
    } finally {
      errLines.mockRestore();
      api[Symbol.dispose]();
      port1.close();
