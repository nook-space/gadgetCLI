...
A gadget is a small app on a Cloudflare OS instance — three files, edited locally,
synced with the CLI.
...
`--json` works on `list`, `status`, `doctor`, `whoami`, in any argument position. Errors
print one `error:` line plus one `hint:`; exit codes are 2 usage, 3 auth, 4 conflict, 5 rpc.
